From v1.2 onwards, there is multi-input capability for P2PKH transactions.<br/>
Here is an example where we create a transaction with 4 utxos. We use the _online_ computer and the script _ONLINE/sign.sh_.<br/>
...
Now it is our task to sign _each_ utxo with the corresponding private key WIF on the __*offline*__ computer:<br/>
...
Note that as a second parameter, we've put a `1` indicating that we signed the first utxo.<br/>
You can see the _sigscript_ added right after the _pkscript_, see above.<br/>
Also note that as per the default setting, the script writes a _new_ XML file each time a signature is added. If you prefer to modify the XML file in-place, specify `--overwrite` as a 3rd parameter.<br/>
Now we sign the second utxo:<br/>
...
Now we sign the 4th and last outpoint:<br/>
